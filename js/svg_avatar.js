/**************************************************************************
 * svgavatars.core.js based on (http://codecanyon.net/user/DeeThemes)
**************************************************************************/
	...
	'<div class="svga-vert-order-colors">',
        '<div class="svga-row row-colors">',
            '<div id="svga-colors-wrap">',
                '<div id="svga-colors" class="scrollbar scroll-simple_outer">',
...
$(document.body).append("<div id=\"draw_functions\" style=\"display:none\"></p>");//functions go here

if (SVG.supported) {
  $('#svgAvatars').empty().html(markup);//want to keep everything
} else {
	markup = [
	'<div id="svga-container" class="svga-no-touch">',
...
$("#draw_functions").data("start", false);
...
$('#svga-start-girls').click(function() {
    $("#draw_functions").data("start", true);
    $("#draw_functions").data("gender", "girls");
    $('#svga-start-boys').removeClass('selected');
    $('#svga-start-girls').addClass('selected');
...
$('#svga-start-boys').click(function() {
    $("#draw_functions").data("start", true);
    $("#draw_functions").data("gender", "boys");
    $('#svga-start-girls').removeClass('selected');
    $('#svga-start-boys').addClass('selected');
...
for (var i = 0; i < body_zone_list.length; i++) {
	...
	shapes_counter = 0;
    SVG.get('svga-group-subwrapper').svgaScaleDown();
};
...
//change zone button
$("#draw_functions").on('changeZone',function(event,zone){
    body_zone = zone;
    $("#feature_button").html('Change '+$('#color_field').val());
...
//draws feature shape (# 0-n) in the zone
$("#draw_functions").on('drawFeature', function(event, cur_zone, shape){
	...
	for (var i = 0; i < cur_zone.length; i++) {
		...
		if (cur_zone[i] === 'facehighlight' || cur_zone[i] === 'humanbody') {
			shapes_counter = 0;
		} else {
			shapes_counter = shape;
		};
	...
	changes_counter = ++changes_counter;
}//wrapper to make accessible globally
);
    
$("#draw_functions").on('getZoneShapes', function(event,zone){
    $("#draw_functions").data("zoneShapes", data[zone].shapes);
});
...
$('.svga-elements').click(function(){
	var $this = $(this),
		el_data = $this.data();
	var cur_zone = el_data.zone;//new local var in this func
	if (cur_zone === 'eyesfront') {
		cur_zone = ['eyesback','eyesfront'];
	} else if (cur_zone === 'faceshape') {
		cur_zone = ['faceshape','chinshadow'];
...
$('.svga-glob-controls').click(function(){
	...
	switch (id) {
		...
		case 'svga-glob-controls-tiltleft': {
			group = SVG.get('svga-group-hair-back');
			group.svgaRotateLeft(1,3,100,150);
			group = SVG.get('svga-group-head');
			group.svgaRotateLeft(1,3,100,150);
			break;
		}
		case 'svga-glob-controls-tiltright': {
			group = SVG.get('svga-group-hair-back');
			group.svgaRotateRight(1,3,100,150);
			group = SVG.get('svga-group-head');
			group.svgaRotateRight(1,3,100,150);
			break;
		}
...
$("#draw_functions").on('tiltHeadLeft',function(event){
    var group = SVG.get('svga-group-hair-back');
	group.svgaRotateLeft(1,3,100,150);
	group = SVG.get('svga-group-head');
	group.svgaRotateLeft(1,3,100,150);
});
$("#draw_functions").on('tiltHeadRight',function(event){
   var  group = SVG.get('svga-group-hair-back');
    group.svgaRotateRight(1,3,100,150);
    group = SVG.get('svga-group-head');
    group.svgaRotateRight(1,3,100,150);
...
$("#draw_functions").on('moveUp',function(event,zone){
    var cur_group;
	switch (zone) {
		case 'mouth': {
...
//move avatar part down
$("#draw_functions").on('moveDown',function(event,zone){
    var cur_group;
	switch (zone) {
		case 'mouth': {
			cur_group = SVG.get('svga-group-mouth-single');
...
$("#draw_functions").on('moveLeft',function(event,zone){
    var cur_group;
	switch (zone) {
		case 'mouth': {
			cur_group = SVG.get('svga-group-mouth-single');
			cur_group.svgaLeft(2,0.5);
			break;
		}
		case 'nose': {
			cur_group = SVG.get('svga-group-nose-single');
...
$('#svga-controls-left').click(function(){
	var cur_group;
	switch (body_zone) {
		case 'mouth': {
			cur_group = SVG.get('svga-group-mouth-single');
...
$("#draw_functions").on('moveRight',function(event,zone){
    var cur_group;
	switch (zone) {
		case 'mouth': {
			cur_group = SVG.get('svga-group-mouth-single');
			cur_group.svgaRight(2,0.5);
			break;
		}
		case 'nose': {
			cur_group = SVG.get('svga-group-nose-single');
...
$('#svga-controls-right').click(function(){
	var cur_group;
	switch (body_zone) {
		case 'mouth': {
			cur_group = SVG.get('svga-group-mouth-single');
...
$("#draw_functions").on('zoomIn',function(event,zone){
    var cur_group;
	switch (zone) {
		case 'faceshape': {
			cur_group = SVG.get('svga-group-faceshape-wrap');
			cur_group.svgaScaleUp(2, 0.02, 0.0001);
			cur_group = SVG.get('svga-group-hair-back');
			cur_group.svgaScaleUp(2, 0.02, 0.0001);
			cur_group = SVG.get('svga-group-hair-front');
			cur_group.svgaScaleUp(2, 0.02, 0.0001);
			if (gender === 'boys') {
				cur_group = SVG.get('svga-group-beardwrap');
				cur_group.svgaScaleUp(2, 0.02, 0.0001);
			};
			cur_group = SVG.get('svga-group-ears-left');
			cur_group.svgaLeft(2,1.5);
			cur_group = SVG.get('svga-group-ears-right');
			cur_group.svgaRight(2,1.5);
			break;
		}
		case 'mouth': {
			cur_group = SVG.get('svga-group-mouth-single');
			cur_group.svgaScaleUp(7);
			break;
		}
		case 'nose': {
			cur_group = SVG.get('svga-group-nose-single');
			cur_group.svgaScaleUp(5);
			break;
		}
		case 'ears': {
			cur_group = SVG.get('svga-group-ears-left');
			cur_group.svgaScaleUp(1);
			cur_group = SVG.get('svga-group-ears-right');
			cur_group.svgaScaleUp(1);
			break;
		}
...
$('#svga-controls-scaleup').click(function(){
	var cur_group;
	switch (body_zone) {
		case 'faceshape': {
			cur_group = SVG.get('svga-group-faceshape-wrap');
...
$("#draw_functions").on('zoomOut',function(event,zone){
    var cur_group;
	switch (zone) {
		case 'faceshape': {
			cur_group = SVG.get('svga-group-faceshape-wrap');
			cur_group.svgaScaleDown(2, 0.02, 0.0001);
			cur_group = SVG.get('svga-group-hair-back');
			cur_group.svgaScaleDown(2, 0.02, 0.0001);
			cur_group = SVG.get('svga-group-hair-front');
			cur_group.svgaScaleDown(2, 0.02, 0.0001);
		...
		case 'mouth': {
			cur_group = SVG.get('svga-group-mouth-single');
			cur_group.svgaScaleDown(7);
			break;
		}
		case 'nose': {
			cur_group = SVG.get('svga-group-nose-single');
			cur_group.svgaScaleDown(5);
			break;
		}
		case 'ears': {
			cur_group = SVG.get('svga-group-ears-left');
			cur_group.svgaScaleDown(1);
			cur_group = SVG.get('svga-group-ears-right');
			cur_group.svgaScaleDown(1);
			break;
		}
...
$("#draw_functions").on('getBoundingBox',function(event,zone){
    var cur_group;
	switch (zone) {
		case 'faceshape': {
			cur_group = SVG.get('svga-group-faceshape-wrap');
            $("#draw_functions").data("BB", cur_group.getBBox());
            console.log(cur_group.getBBox());
			break;
		}
		case 'mouth': {
			cur_group = SVG.get('svga-group-mouth-single');
...
$('#svga-controls-scaledown').click(function(){
	var cur_group;
	switch (body_zone) {
		case 'faceshape': {
			cur_group = SVG.get('svga-group-faceshape-wrap');
...
$("#draw_functions").on('tighten',function(event,zone){
    var cur_group;
	switch (zone) {
		case 'eyebrows': {
			cur_group = SVG.get('svga-group-eyebrows-left');
...
$('#svga-controls-tightly').click(function(){
	var cur_group;
	switch (body_zone) {
		case 'eyebrows': {
			cur_group = SVG.get('svga-group-eyebrows-left');
...
$("#draw_functions").on('widen',function(event,zone){
    var cur_group;
	switch (zone) {
		case 'eyebrows': {
			cur_group = SVG.get('svga-group-eyebrows-left');
...
$('#svga-controls-wider').click(function(){
	var cur_group;
	switch (body_zone) {
		case 'eyebrows': {
			cur_group = SVG.get('svga-group-eyebrows-left');
...
$("#draw_functions").on('eyebrows_1',function(event){
	var cur_group;
	cur_group = SVG.get('svga-group-eyebrows-left');
	cur_group.svgaCancelRotate().svgaRotateRight(1,rotate_step/2);
	cur_group = SVG.get('svga-group-eyebrows-right');
	cur_group.svgaCancelRotate().svgaRotateRight(1,rotate_step/4);
	changes_counter = ++changes_counter;
});
$("#draw_functions").on('eyebrows_2',function(event){
	var cur_group;
	cur_group = SVG.get('svga-group-eyebrows-left');
	cur_group.svgaCancelRotate().svgaRotateLeft(1,rotate_step/4);
	cur_group = SVG.get('svga-group-eyebrows-right');
	cur_group.svgaCancelRotate().svgaRotateLeft(1,rotate_step/2);
	changes_counter = ++changes_counter;
});
$("#draw_functions").on('eyebrows_3',function(event){
	var cur_group;
	cur_group = SVG.get('svga-group-eyebrows-left');
	cur_group.svgaCancelRotate().svgaRotateRight(1,rotate_step/2);
	cur_group = SVG.get('svga-group-eyebrows-right');
...
$("#draw_functions").on('eyebrows_4',function(event){
	var cur_group;
	cur_group = SVG.get('svga-group-eyebrows-left');
	cur_group.svgaCancelRotate().svgaRotateLeft(1,rotate_step/1.4);
	cur_group = SVG.get('svga-group-eyebrows-right');
	cur_group.svgaCancelRotate().svgaRotateRight(1,rotate_step/1.4);
	changes_counter = ++changes_counter;
});
$("#draw_functions").on('eyebrows_cancel',function(event){
	var cur_group;
	cur_group = SVG.get('svga-group-eyebrows-left');
	cur_group.svgaCancelRotate();
	cur_group = SVG.get('svga-group-eyebrows-right');
	cur_group.svgaCancelRotate();
});
...
function randomAvatar() {
	...
	for (var cur_zone in data) {
		...
		if (data.hasOwnProperty(cur_zone)) {
			...
			if (shapes_counter && !(cur_zone === 'glasses' || cur_zone === 'backs' || cur_zone == 'clothes' || cur_zone === 'eyesiris')) {
                var z = body_zone;
				if (cur_zone === 'hair' || cur_zone === 'mustache' || cur_zone === 'beard' || cur_zone === 'eyebrows') {
                    $("#draw_functions").trigger('drawFeature',[cur_zone, shapes_counter]);
					body_zone = cur_zone;
					//$('#svga-colors-' + body_zone + ' > div:nth-child(' + hair_color_counter + ')').trigger('click');
                    //TODO: replace color in color_storage
                    updateColors(data[body_zone].colors[Math.floor(Math.random()*data[body_zone].colors.length)]);
				} else {
					$("#draw_functions").trigger('drawFeature',[cur_zone, shapes_counter]);
					body_zone = cur_zone;
...
$("#draw_functions").on('randomAvatar',function(event){
    randomAvatar();
});    
...
$('#svga-tryagain').click(function() {
	$('#svga-loader').hide();
	$('#svga-message').hide();
	$('#svga-ios').hide();
	$('#svga-share-block').hide();
	$('#svga-gravatar-message').fadeOut('fast');
	$('#svga-gravataravatar').click();
});

//save avatar on a server
$('#svga-saveavatar').click(function(){
	$('#svga-work-overlay').fadeIn('fast');
	$('#svga-canvas').attr({width: save_size, height: save_size});
	if (rect) {rect.hide();};
	if (text) {text.hide();};
	var svg = $('#svga-svgmain').html();
	svg = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="' + save_size + 'px" height="' + save_size + 'px" xlink="http://www.w3.org/1999/xlink" style="overflow:hidden!important;"><g id="svga-group-wrapper" transform="scale(' + save_size/200 + ' ' + save_size/200 + ')">'
		+
		svg.replace(/<svg(.*?)>/, '').replace(/<g id(.*?)>/, '').replace(/data-(.*?)\"(.*?)\"/g, '').replace(/   /g, ' ').replace(/  /g, ' ').replace(/  /g, ' ');
	if (save_format === 'png') {
	...
	$('#svga-canvas').attr({width: 200, height: 200});
	$.ajax({
		url: path_to_folder + 'php/save-ready-avatar.php',
		type: 'post',
		dataType: 'text',
		data: { 
			imgdata: img,
		...
		success: function(response){
			if (response == 'saved') {
				...
				$('#svga-work-overlay').fadeIn('fast');
				$('#svga-message-text').html(alert_error).addClass('svga-error');
				$('#svga-message').fadeIn('fast');
			}
		},
		error: function(){
...
$('#svga-gravataravatar').click(function(){
	$('#svga-work-overlay').fadeIn('fast');
	$('#svga-gravatar').fadeIn('fast');
});
$('#svga-gravatar-cancel').click(function(){
	$('#svga-gravatar').hide();
	$('#svga-work-overlay').fadeOut('fast');
});
$('#svga-gravatar-ok').click(function(){
	$('#svga-gravatar').hide();
	$('#svga-loader').show();
	$('#svga-canvas').attr({width: gravatar_size, height: gravatar_size});
	if (rect) {rect.hide();};
	if (text) {text.hide();};
	var svg = $('#svga-svgmain').html();
	svg = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="' + gravatar_size+ 'px" height="' + gravatar_size+ 'px" xlink="http://www.w3.org/1999/xlink" style="overflow:hidden!important;"><g id="svga-group-wrapper" transform="scale(' + gravatar_size/200 + ' ' + gravatar_size/200 + ')">'
		+
		svg.replace(/<svg(.*?)>/, '').replace(/<g id(.*?)>/, '').replace(/data-(.*?)\"(.*?)\"/g, '').replace(/   /g, ' ').replace(/  /g, ' ').replace(/  /g, ' ');
	var canvas = document.getElementById('svga-canvas');
	canvg(canvas, svg, { ignoreMouse: true, ignoreDimensions: true });
	...
	$.ajax({
		url: path_to_folder + "php/send-to-gravatar.php",
		type: "post",
		dataType: 'text',
		data: { 
			imgdata: img,
		...
		success: function(response) {
			if (response == 'success') {
				...
				$('#svga-loader').hide();
				$('#svga-message-text').html(alert_success_gravatar);
				$('#svga-message').fadeIn('fast');
			} else {
				switch (response) {
					case 'ratingfail': {
						overlayMessage(alert_error_ratingfail);
						//gravatar_modal = true;
						break;
					}
					case 'emailfail': {
						overlayMessage(alert_error_emailfail);
						//gravatar_modal = true;
						break;
					}
					case 'passwordfail': {
						overlayMessage(alert_error_passwordfail);
						break;
					}
					case 'imagefail': {
						overlayMessage(alert_error_imagefail);
						break;
					}
					case 'faultcode-8': {
						$('#svga-loader').hide();
						$('#svga-message-text').html(alert_error_faultcode_8).addClass('svga-error');
						$('#svga-work-overlay').fadeIn('fast');
						$('#svga-message').fadeIn('fast');
						break;
					}
					case 'faultcode-9': {
						overlayMessage(alert_error_faultcode_9);
						break;
					}
					default: {
						overlayMessage(alert_common_error_gravatar);
						break;
					}
				};
			}
...
$('#svga-png-one').click(function(){
	$('#svga-work-overlay').fadeIn('fast');
	$('#svga-canvas').attr({width: png_one_download_size, height: png_one_download_size});
	if (rect) {rect.hide();};
	if (text) {text.hide();};
	var svg = $('#svga-svgmain').html();
	svg = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="' + png_one_download_size+ 'px" height="' + png_one_download_size+ 'px" xlink="http://www.w3.org/1999/xlink" style="overflow:hidden!important;"><g id="svga-group-wrapper" transform="scale(' + png_one_download_size/200 + ' ' + png_one_download_size/200 + ')">'
		+
		svg.replace(/<svg(.*?)>/, '').replace(/<g id(.*?)>/, '').replace(/data-(.*?)\"(.*?)\"/g, '').replace(/   /g, ' ').replace(/  /g, ' ').replace(/  /g, ' ');
	var canvas = document.getElementById('svga-canvas');
	canvg(canvas, svg, { ignoreMouse: true, ignoreDimensions: true });
	...
	$('#svga-canvas').attr({width: 200, height: 200});
	$.ajax({
		url: path_to_folder + "php/save-temp-avatar.php",
		type: "post",
		dataType: 'text',
		data: { 
			imgdata: img,
		...
		success: function(response){
			if (response == 'saved') {
				...
				$('#svga-message-text').html(alert_error_download).addClass('svga-error');
				$('#svga-work-overlay').fadeIn('fast');
				$('#svga-message').fadeIn('fast');
			}
		},
...
$('#svga-png-two').click(function(){
	$('#svga-work-overlay').fadeIn('fast');
	$('#svga-canvas').attr({width: png_two_download_size, height: png_two_download_size});
	if (rect) {rect.hide();};
	if (text) {text.hide();};
	var svg = $('#svga-svgmain').html();
	svg = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="' + png_two_download_size+ 'px" height="' + png_two_download_size+ 'px" xlink="http://www.w3.org/1999/xlink" style="overflow:hidden!important;"><g id="svga-group-wrapper" transform="scale(' + png_two_download_size/200 + ' ' + png_two_download_size/200 + ')">'
		+
		svg.replace(/<svg(.*?)>/, '').replace(/<g id(.*?)>/, '').replace(/data-(.*?)\"(.*?)\"/g, '').replace(/   /g, ' ').replace(/  /g, ' ').replace(/  /g, ' ');
	var canvas = document.getElementById('svga-canvas');
	canvg(canvas, svg, { ignoreMouse: true, ignoreDimensions: true });
...
//get result PNG file
$("#draw_functions").on('createPNG',function(event){
    ...
    $("#draw_functions").data("PNG", img);
});

//get result PNG file on iOS devices
if (iOS) {
	$('#svga-downloadavatar').click(function(){
		$('#svga-work-overlay').fadeIn('fast');
		$('#svga-canvas').attr({width: png_ios_download_size, height: png_ios_download_size});
		if (rect) {rect.hide();};
		if (text) {text.hide();};
		var svg = $('#svga-svgmain').html();
		svg = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="' + png_ios_download_size+ 'px" height="' + png_ios_download_size+ 'px" xlink="http://www.w3.org/1999/xlink" style="overflow:hidden!important;"><g id="svga-group-wrapper" transform="scale(' + png_ios_download_size/200 + ' ' + png_ios_download_size/200 + ')">'
			+
			svg.replace(/<svg(.*?)>/, '').replace(/<g id(.*?)>/, '').replace(/data-(.*?)\"(.*?)\"/g, '').replace(/   /g, ' ').replace(/  /g, ' ').replace(/  /g, ' ');
		var canvas = document.getElementById('svga-canvas');
		canvg(canvas, svg, { ignoreMouse: true, ignoreDimensions: true });
			...
			success: function(response){
				if (response == 'saved') {
					...
					var ios_img = $('<img />')
						.error(function() {
							$( this ).hide();
							$('#svga-message-text').html(alert_error_image).addClass('svga-error');
							$('#svga-work-overlay').fadeIn('fast');
							$('#svga-message').fadeIn('fast');
						})
						.attr('src', path_to_folder + 'temp-avatars/'+filename)
						.on('load', function() {
							if (!this.complete || typeof this.naturalWidth == "undefined" || this.naturalWidth == 0) {
								...
								$("#svga-ios-image").empty().append(ios_img);
								$('#svga-work-overlay').fadeIn('fast');
...
if (Win8tablet) {
	$('#svga-downloadavatar').click(function(){
		$('#svga-work-overlay').fadeIn('fast');
		$('#svga-canvas').attr({width: png_win8tablet_download_size, height: png_win8tablet_download_size});
		if (rect) {rect.hide();};
		if (text) {text.hide();};
		var svg = $('#svga-svgmain').html();
		svg = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="' + png_win8tablet_download_size+ 'px" height="' + png_win8tablet_download_size+ 'px" xlink="http://www.w3.org/1999/xlink" style="overflow:hidden!important;"><g id="svga-group-wrapper" transform="scale(' + png_win8tablet_download_size/200 + ' ' + png_win8tablet_download_size/200 + ')">'
			+
			svg.replace(/<svg(.*?)>/, '').replace(/<g id(.*?)>/, '').replace(/data-(.*?)\"(.*?)\"/g, '').replace(/   /g, ' ').replace(/  /g, ' ').replace(/  /g, ' ');
		var canvas = document.getElementById('svga-canvas');
		canvg(canvas, svg, { ignoreMouse: true, ignoreDimensions: true });
...
//get result SVG file
$('#svga-svgfile').click(function(){
	$('#svga-work-overlay').fadeIn('fast');
	if (rect) {rect.hide();};
	if (text) {text.hide();};
	var svg = $('#svga-svgmain').html();
	svg = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="' + svg_download_size + 'px" height="' + svg_download_size + 'px" xlink="http://www.w3.org/1999/xlink" style="overflow:hidden!important;"><g id="svga-group-wrapper" transform="scale(' + svg_download_size/200 + ' ' + svg_download_size/200 + ')">'
		+
		svg.replace(/<svg(.*?)>/, '').replace(/<g id(.*?)>/, '').replace(/data-(.*?)\"(.*?)\"/g, '').replace(/   /g, ' ').replace(/  /g, ' ').replace(/  /g, ' ');
	var filename = 'svgA' + (Math.random() + '').replace('0.', '') + '.svg';
	$.ajax({
		url: path_to_folder + "php/save-temp-avatar.php",
		type: "post",
		dataType: 'text',
...
$('#svga-shareavatar').click(function(){
	$('#svga-work-overlay').fadeIn('fast');
	$('#svga-canvas').attr({width: share_image_size, height: share_image_size});
	if (rect) {rect.show();};
	if (text) {text.show();};
	var svg = $('#svga-svgmain').html();
	svg = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="' + share_image_size+ 'px" height="' + share_image_size+ 'px" xlink="http://www.w3.org/1999/xlink" style="overflow:hidden!important;"><g id="svga-group-wrapper" transform="scale(' + share_image_size/200 + ' ' + share_image_size/200 + ')">'
		+
		svg.replace(/<svg(.*?)>/, '').replace(/<g id(.*?)>/, '').replace(/data-(.*?)\"(.*?)\"/g, '').replace(/   /g, ' ').replace(/  /g, ' ').replace(/  /g, ' ');
	var canvas = document.getElementById('svga-canvas');
	canvg(canvas, svg, { ignoreMouse: true, ignoreDimensions: true });
				...
				var share_img = $('<img />')
					...
					.on('load', function() {
						if (!this.complete || typeof this.naturalWidth == "undefined" || this.naturalWidth == 0) {
							...
							$("#svga-share-image").empty().append(share_img);
							$('#svga-work-overlay').fadeIn('fast');
...
function shareFacebook() {
	var share_img = $('#svga-share-image img')[0].src;
	window.open('https://www.facebook.com/dialog/feed?app_id=' + facebook_app_id + '&link=' + encodeURIComponent(share_link) + '&picture=' + encodeURIComponent(share_img) + '&name=' + encodeURIComponent(share_title) + '&display=popup&description=' + encodeURIComponent(share_description) + '&redirect_uri=https://www.facebook.com','Facebook','menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');
};
function shareTwitter() {
	window.open('https://twitter.com/intent/tweet?original_referer=' + encodeURIComponent(share_link) + '&text=' + encodeURIComponent(share_title) + '%20' + encodeURIComponent(share_link),'Twitter','menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');
};
function sharePinterest() {
	var share_img = $('#svga-share-image img')[0].src;
	window.open('//pinterest.com/pin/create/button/?url=' + encodeURIComponent(share_link) + '&media=' + encodeURIComponent(share_img) + '&description=' + encodeURIComponent(share_title),'Pinterest','menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');
};
function shareGoogleplus() {
	window.open('//plus.google.com/share?url=' + encodeURIComponent(share_link),'GooglePlus','menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');
};
$("#svga-share-facebook").click(function() {
	shareFacebook();
	return false;
});
$("#svga-share-twitter").click(function() {
	shareTwitter();
	return false;
});
$("#svga-share-pinterest").click(function() {
	sharePinterest();
	return false;
});
$("#svga-share-googleplus").click(function() {
	shareGoogleplus();
	return false;
});

//func to force download a previously saved avatar from a temporary folder
function getFile(temp_file, file_type) {
	var fD = new fileDownloader(path_to_folder + 'php/download-temp-avatar.php');
	fD.fileRequest(temp_file, file_type);
	function fileDownloader(url) {
		this.url = url;
		this.fileRequest = function(temp_file, file_type) {
			if(!url) return;
			var filename_input = document.createElement("input");
			filename_input.setAttribute("name", 'filename');
			filename_input.setAttribute("value", temp_file);
			var filetype_input = document.createElement("input");
			filetype_input.setAttribute("name", 'filetype');
			filetype_input.setAttribute("value", file_type);
			var temp_form = document.createElement("form");
			temp_form.method = 'get';
			temp_form.action = url;
			temp_form.target = '_top';
			temp_form.appendChild(filename_input);
			temp_form.appendChild(filetype_input);
