...
function submitElement(){
    ...
    if(face_part == "Face"){
        ...
        for(var point_key in sketches[face_part].getPoints()){
            var One_point = {x:sketches[face_part].getPoints()[point_key].getX(),y:sketches[face_part].getPoints()[point_key].getY()};
            points_OnePart.push(One_point);
        }
        
        //var hull=convexHull(points_OnePart);
        //var hull=concaveman(points_OnePart);
        var hull=hull(points_OnePart,20);
        var avatar_canvas = document.getElementById('avatar');//canvas for avatar
        var ctx = avatar_canvas.getContext('2d');
        ctx.beginPath();
        for(var point_key=0; point_key < hull.length-1; point_key++){
            ctx.moveTo(hull[point_key].x,hull[point_key].y);
            ctx.lineTo(hull[point_key+1].x,hull[point_key+1].y);
